...
public class devent {
	...
	public static void setup() {
			...
			public void process(String body) {
				...
				if (sc != null) {
					...
					if (sys.panel.mode == 1) //围墙机
						...
						SysAccess.elev(f, r);
					for(int k=0; k<sc.ext.size(); k++) {
						try {
							Thread.sleep(50);
						} catch (InterruptedException e) {
						}
						int family = sc.ext.get(k).intValue();
						f = family / 100;
						r = family % 100;
						if (sys.panel.mode == 1) //围墙机
							SysAccess.elevWall(b, u, f, r);
		...
		de = new devent("/ui/wx/card/download") { //微云门禁卡号下发
			@Override
			public void process(String body) {
				dmsg.ack(200, null);
				WxCard.load();
