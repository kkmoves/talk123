...
public class SDTLoggerLabel extends BaseLabel {
	...
	private long mData[] = new long[SDTLogger.MAX];
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		b2.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				DatePickerDialog d = new DatePickerDialog(SDTLoggerLabel.this, new OnDateSetListener() {
					@Override
	...
	public void loadData(long start, long end) {
		int n = SDTLogger.mIdx - 1;
		mCount = 0;
		while (n >= 0) {
			if (SDTLogger.mData[n] >= start && SDTLogger.mData[n] <= end) {
				mData[mCount] = SDTLogger.mData[n];
				mCount++;
			}
			n--;
		}
	...
	public void showData() {
		for (int i = 0; i < MAX; i++) {
			if (mStart + i < mCount) {
				SDTLogger.Data d = SDTLogger.load(mData[mStart + i]);
	...
	private final class RowOnClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			if (n != -1 && mStart+n < mCount) {
				SDTViewLabel.mMode = 0;
				SDTViewLabel.mData = mData[mStart+n];
				Intent it = new Intent(SDTLoggerLabel.this, SDTViewLabel.class);
